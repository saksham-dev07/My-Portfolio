...
export const ThemeMoodProvider = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (saved && ["cyber", "light", "hacker", "chill", "chaotic"].includes(saved)) {
        setMoodState(saved);
  ...
  const setMood = useCallback((newMood, eventOrCoords) => {
    if (!["cyber", "light", "hacker", "chill", "chaotic"].includes(newMood)) return;
    ...
    if (eventOrCoords) {
      ...
      } else if (typeof eventOrCoords.clientX === "number" && typeof eventOrCoords.clientY === "number" && (eventOrCoords.clientX !== 0 || eventOrCoords.clientY !== 0)) {
  ...
  const toggleLightDark = useCallback((eventOrCoords) => {
    const nextMood = mood === "light" ? "cyber" : "light";
    setMood(nextMood, eventOrCoords);
  }, [mood, setMood]);

  return (
    <ThemeMoodContext.Provider value={{ mood, setMood, toggleLightDark, moods: MOODS }}>
      {children}
